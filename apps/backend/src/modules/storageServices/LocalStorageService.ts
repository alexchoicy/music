...
export class LocalStorageService extends StorageService {
	...
	getMusicDataUrl(trackHash: string, quality: TrackQualityType, ext: string) {
		return `${this.config.get('appConfig.app.public_data_url')}/media/music/${quality}/${trackHash}${ext}`;
	}

	getAlbumCoverDataUrl(attachmentID: string, ext: string) {
		return `${this.config.get('appConfig.app.public_data_url')}/media/cover/${attachmentID}.${ext}`;
	}
