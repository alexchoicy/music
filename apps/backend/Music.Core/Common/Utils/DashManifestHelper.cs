...
public static class DashManifestHelper
{
    ...
    public static void InjectAudioLabelsIntoDashManifest(
        string manifestPath,
    ...
    )
    {
        ...
        XElement root =
            document.Root ?? throw new InvalidOperationException("MPD root element not found.");
        XNamespace ns = root.Name.Namespace;

        List<XElement> audioAdaptationSets = root.Descendants(ns + "AdaptationSet")
    ...
    private static void ApplyAudioStreamMetadata(
        XElement element,
    ...
    public static string InjectPresignUrl(
        string xml,
        string objectFolder,
    ...
    )
    {
        ...
        if (value.Contains('$', StringComparison.Ordinal))
        {
            throw new InvalidOperationException(
                "Manifest contains unresolved DASH template placeholder in BaseURL."
            );
        }
