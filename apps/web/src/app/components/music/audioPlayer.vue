...
<template>
  <div class="sticky bottom-0 z-20 bg-background border-t border-border">
    <!-- Mobile -->
    <div class="flex items-center justify-between py-3 px-3 md:hidden">
      ...
      <div class="flex items-center gap-2">
        <div class="flex flex-col text-left overflow-hidden w-36">
          <div class="text-sm font-semibold text-foreground truncate" :title="audioPlayer.currentTrack?.name">
            {{ audioPlayer.currentTrack?.name }}
          </div>
          <div class="text-xs text-muted-foreground truncate"
            :title="audioPlayer.currentTrack?.artists.map((a) => a.name).join(', ')">
            {{audioPlayer.currentTrack?.artists.map((a) => a.name).join(", ")}}
          </div>
        </div>

        <Button variant="ghost" @click="props.togglePlayList" aria-label="Options">
          <ListMusic />
        </Button>
      </div>
    </div>
    ...
    <!-- Desktop -->
    <div class="hidden md:flex flex-row py-4 px-2 items-center">
      ...
      <div class="flex items-center justify-end m-auto">
        ...
        <Tooltip>
          ...
          <TooltipContent>
            <p class="p-1">Playing in {{ audioPlayer.quality }} Quality</p>
          </TooltipContent>
        </Tooltip>
      </div>
    </div>
