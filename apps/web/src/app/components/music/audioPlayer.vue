...
<template>
  <div class="sticky bottom-0 z-20 bg-background flex flex-row py-4 px-2 border-t border-border">
    <div class="flex flex-row item-center justify-center m-auto">
      ...
      <Button
        variant="ghost"
        :class="audioPlayer.isShuffling ? 'text-primary' : 'text-muted-foreground'"
    ...
    <div
      class="flex flex-1 items-center gap-3 rounded-lg p-2 hover:bg-accent dark:hover:bg-accent/50"
      @click="onClickInfo">
      <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-gray-700 md:h-12 md:w-12">
        <img
          v-if="audioPlayer.currentTrack?.album.cover"
          :src="audioPlayer.currentTrack?.album.cover"
          alt="Album cover"
      ...
      <div class="flex flex-1 flex-col text-left overflow-hidden w-40 md:w-64">
        ...
        <div
          class="text-xs text-muted-foreground md:block whitespace-nowrap"
