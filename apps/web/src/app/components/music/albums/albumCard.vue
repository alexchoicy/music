  ...
  <Card
    ...
    <CardContent>
      <div class="aspect-square relative overflow-hidden">
        ...
        <Badge variant="secondary" class="absolute top-2 right-2 bg-black/60 text-white border-0 backdrop-blur-sm">
          {{ album.albumType }}
        </Badge>
        <Button
          size="icon"
      ...
      <div class="p-4 space-y-1">
        ...
        <div class="flex gap-1 flex-wrap">
          <Badge v-if="album.hasInstrumental" variant="outline" class="opacity-60">Instrumental</Badge>
        </div>
        <p class="text-sm text-muted-foreground">
          <span v-if="album.year != 0">{{ album.year }} •</span>
