...
<template>
  <div class="flex items-center gap-4 w-full">
    <div class="w-16 h-16 rounded-lg flex flex-row item-center justify-center shrink-0">
      <img
        v-if="album.disc[0]!.musics?.[0]?.picture?.[0]?.data"
      ...
      <p class="text-gray-400">
        ...
        <Tooltip v-if="isFeatUndetected">
          ...
          <TooltipContent side="top" class="max-w-xs">
            <p>
              It seems that this track might contain a featured artist, but it was not solved. Please adding the
              featured artist to the using track editor and remove the ft
            </p>
          </TooltipContent>
        </Tooltip>
        <Tooltip v-if="isVariousArtists">
          ...
          <TooltipContent side="top" class="max-w-xs">
            <p>
              I suggest to not store the albums as Various Artists, change the main Artist name to this albums's project
              name and mark it to "project".
            </p>
          </TooltipContent>
        </Tooltip>
      </p>
    ...
    <div>
      <Button variant="ghost" class="h-9 w-9 p-0" :disabled="props.blockUpload" @click="onAlbumEditOpen(album.hash)">
