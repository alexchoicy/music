<script setup lang="ts">
  ...
  watch(
    () => [props.currentTrack, props.isOpen],
    async ([currentTrack, isOpen]) => {
  ...
  const trackEditFormSchema = toTypedSchema(
    z.object({
  ...
  const {
    ...
    remove: artistsFieldsRemove,
    push: artistsFieldsPush,
  } = useFieldArray<string>("artists");
  ...
  <Dialog :open="isOpen" @update:open="handleAlbumEditDialogOpenChange">
    <DialogScrollContent>
      ...
      <form class="space-y-4" @submit="onTrackFormSubmit">
        ...
        <FormField
          v-for="(artistField, index) in artistsFields"
          :key="artistField.key"
        ...
        <FormField v-slot="{ componentField }" name="album">
          <FormItem>
            ...
            <FormDescription>All track should have the same album name.</FormDescription>
          </FormItem>
        </FormField>
        ...
        <FormField v-slot="{ value, handleChange }" type="checkbox" name="isInstrumental">
          <FormItem class="flex flex-row items-start gap-x-3 space-y-0 rounded-md border p-4 shadow">
            ...
            <div class="space-y-1 leading-none">
              ...
              <FormDescription>Don't apply if it is part of a compilation album.</FormDescription>
              <FormMessage />
            </div>
          </FormItem>
        </FormField>
        <FormField v-slot="{ value, handleChange }" type="checkbox" name="isMC">
          <FormItem class="flex flex-row items-start gap-x-3 space-y-0 rounded-md border p-4 shadow">
            ...
            <div class="space-y-1 leading-none">
              ...
              <FormDescription>Check this if the track is a MC (Master of Ceremony) part.</FormDescription>
              <FormMessage />
            </div>
          </FormItem>
        </FormField>
