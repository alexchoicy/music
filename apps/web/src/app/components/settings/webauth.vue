<script setup lang="ts">
  ...
  const startWebAuthRegistration = async () => {
    ...
    const options = await useNuxtApp().$backend<PublicKeyCredentialCreationOptionsJSON>(
      "/auth/webauth/options-registration",
    );
    ...
    const verificationResp = await useNuxtApp().$backend<{ verified: boolean; newDeviceId: string }>(
      "/auth/webauth/verify-registration",
      {
        method: "POST",
        body: attResp,
      },
  ...
  function getAuthTypeIcon(device: string[], deviceType: string): AuthTypeIcon {
    const isDevice =
      device.includes("platform") || device.includes("internal") || deviceType.toLowerCase() === "multiDevice";
    const isKey =
  ...
  <div class="space-y-6">
    <Card>
      <CardHeader>
        ...
        <CardDescription>Register a WebAuthn authenticator for passwordless login.</CardDescription>
      </CardHeader>
      <CardContent>
        <div class="space-y-2">
        ...
        <Button class="mt-4" @click="startWebAuthRegistration">Register WebAuthn Authenticator</Button>
      </CardContent>
    </Card>
    ...
    <Card>
      ...
      <CardContent>
        ...
        <Item v-for="device in devices.data.value" v-else :key="device.id">
          <ItemMedia>
            <Smartphone v-if="getAuthTypeIcon(device.device || [], device.deviceType) === AuthTypeIcon.DEVICE" />
          ...
          <ItemContent>
            ...
            <ItemDescription>
              ...
              <span v-if="device.lastUsedAt">| Last used: {{ formatDate(new Date(device.lastUsedAt)) }}</span>
            </ItemDescription>
          </ItemContent>
          <ItemActions>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            ...
            <br />
            Device: {{ selectedDevice?.device?.join(", ") }}
            <br />
            Created At: {{ selectedDevice ? formatDate(new Date(selectedDevice.createdAt)) : "" }}
            <br />
