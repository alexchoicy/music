<script setup lang="ts">
  ...
  const startWebAuthLogin = async (e: Event) => {
    e.preventDefault();
    const options = await useNuxtApp().$backend<PublicKeyCredentialRequestOptionsJSON>(
      "/auth/webauth/options-authentication",
    );
  ...
  <Card class="w-full max-w-sm">
    <form @submit="onFormSubmit">
      ...
      <CardFooter class="flex flex-col gap-2 p-6 pt-0">
        ...
        <Button variant="outline" class="w-full" @click="startWebAuthLogin">WebAuthhhhh PassKeyyyyyyyyyy</Button>
      </CardFooter>
    </form>
  </Card>
</template>
