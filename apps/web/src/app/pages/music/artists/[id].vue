<script setup lang="ts">
  ...
  const albumOnly = computed(() => data.value?.albums?.filter((album) => album.albumType === "Album") ?? []);
  const singleOnly = computed(() => data.value?.albums?.filter((album) => album.albumType === "Single") ?? []);
  const featuredInOnly = computed(() => data.value?.featuredIn ?? []);
</script>
...
<template>
  ...
  <div v-if="data && !bot">
    ...
    <div>
      <MusicArtistsTabsOverview
        v-if="currentTab === 'overview'"
        :album-only="albumOnly"
        :single-only="singleOnly"
        :featured-in-only="featuredInOnly"
        :group-member="data.groupMembers!"
