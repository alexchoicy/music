...
const albumOnly = props.data.albums.filter(
  (album) => album.albumType === "Album",
);
const singleOnly = props.data.albums.filter(
  ...
  <div class="w-full h-full">
      ...
      <div class="relative h-full flex items-end">
        <div class="container mx-auto px-6 pb-8">
          <div class="flex items-end gap-6">
            ...
            <div class="pb-4">
              <h1 className="text-5xl font-bold mb-2 text-balance">
                {{ data.name }}
              </h1>
            </div>
          </div>
    ...
    <div class="p-10">
      <ArtistOverall
        ...
        :albumOnly="albumOnly.slice(0, 6)"
        :singleOnly="singleOnly.slice(0, 6)"
        :getAlbumInfo="getAlbumInfo"
        :selectTab="selectTab"
      />
      <ArtistAlbum
        v-else-if="currentTab === 'albums'"
        :album="albumOnly"
        :getAlbumInfo="getAlbumInfo"
      />
      <ArtistAlbum
        v-else-if="currentTab === 'single'"
        :album="singleOnly"
        :getAlbumInfo="getAlbumInfo"
      />
    </div>
